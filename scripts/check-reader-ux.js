...
const figures = [
  {
    ...
    anchor: 'figure-ch05-minimum-architecture',
    asset: 'ch05-minimum-architecture.svg',
    indexHref: '../../chapters/chapter-05/#figure-ch05-minimum-architecture'
  },
  {
    source: 'docs/chapters/chapter-05/index.md',
    anchor: 'figure-ch05-order-flow',
    asset: 'ch05-order-flow.svg',
    indexHref: '../../chapters/chapter-05/#figure-ch05-order-flow'
  },
  {
    source: 'docs/chapters/chapter-05/index.md',
    anchor: 'figure-ch05-order-sequence',
    asset: 'ch05-order-sequence.svg',
    indexHref: '../../chapters/chapter-05/#figure-ch05-order-sequence'
  },
  {
    source: 'docs/appendices/templates/architecture-diagram/index.md',
    anchor: 'figure-appendix-architecture-template',
    asset: 'appendix-architecture-template.svg',
    indexHref: '../templates/architecture-diagram/#figure-appendix-architecture-template'
  },
  {
    source: 'docs/appendices/templates/architecture-diagram/index.md',
    anchor: 'figure-appendix-architecture-example',
    asset: 'appendix-architecture-example.svg',
    indexHref: '../templates/architecture-diagram/#figure-appendix-architecture-example'
  }
];
...
function walkMarkdown(dir) {
  ...
  const result = [];
  const entries = fs.readdirSync(full, { withFileTypes: true }).sort(function (left, right) {
...
expect(appendices.filter(function (item) { return item.id === 'figure-index' && item.title === '図表索引'; }).length === 1,
  'book-config.json: structure.appendices must contain one figure-index entry');
...
expect(index.includes('title: "図表索引"') && index.includes('order: 940'), 'figure index: expected front matter');
...
expect(css.includes('.book-figure img') && css.includes('.figure-index-list'),
  'CSS: responsive figure and index rules are required');
expect(/@media \(max-width: 640px\)\s*\{[\s\S]*?\.figure-index-list li\s*\{[\s\S]*?padding:\s*0\.75rem;/.test(css),
  'mobile CSS: narrow viewport must contain the figure-index padding rule');
...
const mermaidMarker = String.fromCharCode(96).repeat(3) + 'mermaid';
const mermaidCounts = markdown.map(function (pair) { return [pair[0], count(pair[1], mermaidMarker)]; })
  .filter(function (pair) { return pair[1] > 0; });
const totalMermaid = mermaidCounts.reduce(function (sum, pair) { return sum + pair[1]; }, 0);
expect(totalMermaid === figures.length, 'public Mermaid inventory must be exactly ' + figures.length + ', got ' + totalMermaid);
expect(JSON.stringify(mermaidCounts) === JSON.stringify([
  ['docs/appendices/templates/architecture-diagram/index.md', 2],
  ['docs/chapters/chapter-05/index.md', 3]
]), 'public Mermaid sources must remain exact 2 appendix + 3 chapter');

for (const sourceFile of ['docs/chapters/chapter-05/index.md', 'docs/appendices/templates/architecture-diagram/index.md']) {
  const source = read(sourceFile);
  const sourceFigures = figures.filter(function (figure) { return figure.source === sourceFile; });
  let cursor = -1;
  for (const figure of sourceFigures) {
    const mermaidPosition = source.indexOf(mermaidMarker, cursor + 1);
    const anchorPosition = source.indexOf('id="' + figure.anchor + '"', mermaidPosition + 1);
    expect(mermaidPosition > cursor && anchorPosition > mermaidPosition,
      sourceFile + ': each Mermaid source must be followed by its contracted preview ' + figure.anchor);
...
expect(!figureRouteSources.includes('/engineering-documentation-book/'),
  'figure sources and index must use baseurl-independent relative links');
...
for (const figure of figures) {
  ...
  const blockPattern = new RegExp('<figure\\b[^>]*\\sid="' + figure.anchor + '"[^>]*>[\\s\\S]*?<\\/figure>');
  const blockMatch = source.match(blockPattern);
  expect(Boolean(blockMatch), figure.source + ': figure block is missing for ' + figure.anchor);
  if (blockMatch) {
    expect(/alt="[^"]{12,}"/.test(blockMatch[0]), figure.source + ': meaningful alt is required for ' + figure.anchor);
...
console.log('Reader UX check passed: 5 Mermaid sources, SVG previews, anchors and index entries.');
